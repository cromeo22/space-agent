...
- keeps Windows NSIS packaging configured to recreate desktop shortcuts on reinstall, pins an explicit canonical `nsis.guid`, and routes legacy rebrand-era uninstall-key migration through `packaging/platforms/windows/installer.nsh` so updater-driven installs keep a stable Windows identity instead of drifting with `appId` changes
