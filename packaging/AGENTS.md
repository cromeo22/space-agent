...
`packaging/desktop/updater_install_options.js` is the canonical packaged-desktop helper for updater handoff strategy. It keeps packaged Windows explicit installs on a detached PowerShell-to-NSIS handoff that waits for the current app executable to disappear before it launches the downloaded installer, because `electron-updater` starts NSIS before Electron has fully exited and that can strand Windows installs half-uninstalled. That helper also owns the stable packaged updater log path `<userData>/logs/desktop-updater.log`, and both the main process and detached Windows handoff append best-effort diagnostics there.
